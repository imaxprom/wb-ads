...
export const COLUMNS: Col[] = [
  ...
  { key: "ordersSum", label: <span className="inline-flex items-center gap-1"><EyeIcon className="w-3.5 h-3.5" /><CartIcon className="w-3.5 h-3.5" /><BoxIcon className="w-3.5 h-3.5" /><span className="w-3.5 h-3.5 inline-flex items-center justify-center text-[13px] leading-none font-semibold">₽</span></span>, tooltip: "Воронка продаж (гибрид: MAX из открытого и закрытого API)\n\nПоказы — показы карточки в выдаче (из закрытого API Джем)\nКорзины — добавления в корзину (общие: орг. + рекл.)\nЗаказы — заказы, шт.\nСумма заказов — по ценам поставки\nВыкупы — выкупленные заказы (из закрытого API Джем)\n\nКлик — сортировка по Сумме Заказов", sortable: true, defaultW: 130, align: "right" },
