...
const SECTIONS: Section[] = [
  {
    ...
    title: "Архитектура",
    items: [
      { label: "Стек", value: "Next.js 16 + TypeScript + Tailwind CSS 4 + PostgreSQL prod + SQLite compatibility/migration scripts + Puppeteer" },
      { label: "Порт", value: "3001" },
      { label: "Prod", value: "ads.imaxprom.site: приложение на VM108, worker wb-ads-worker, основная БД VM107 PostgreSQL wb_ads_prod" },
      { label: "БД", value: "Prod PostgreSQL wb_ads_prod — 50 таблиц, 374 MB на 29 мая 2026. data/ads.db — legacy/dev snapshot, не главный источник истины" },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "/api/sync/products", value: "Карточки + цены + рейтинг → Content + Prices API + sales-funnel API. Цены берутся через отдельный read-only data/wb-prices-api-key.txt; fallback только при явной ошибке Prices API" },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "RPC", value: "~/wb-parser/positions_rpc.py — JSON stdin → JSON stdout. Вызовы централизованы в src/lib/wb-parser-rpc.ts, host override через WB_PARSER_SSH_HOST" },
      ...
      { label: "Endpoints в проекте", value: "/api/sync/phrase-positions-batch, /api/sync/phrase-position-one, /api/sync/phrase-positions, /api/clusters/scan-campaign. На VM108 alias wb-parser проверен отдельным ключом" },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Fail semantics", value: "Полный провал scan больше не пишется как success: если scanned=0 и failed=total, endpoint возвращает ошибку. Проверенный prod-run 25141382: 488/488, failed=0, passes=2" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Кампании (на 29 мая 2026)", value: "336 всего в prod БД" },
      { label: "Manual clusters", value: "1974 кластера в prod БД после массовой кластеризации/импорта и scan-campaign. Каждый имеет UNIQUE preset_id, если preset_id заполнен" },
    ],
  },
];
